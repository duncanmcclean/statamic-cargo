...
use DuncanMcClean\Cargo\Http\Resources\CP\Coupons\Coupons;
use DuncanMcClean\Cargo\Rules\UniqueCouponValue;
...
class CouponController extends CpController
{
    ...
    public function index(FilteredRequest $request)
    {
        ...
        if ($request->wantsJson()) {
            ...
            return (new Coupons($coupons))
                ...
                ->columnPreferenceKey('cargo.coupons.columns')
                ->additional(['meta' => [
        ...
        $columns = $blueprint->columns()
            ->setPreferred('cargo.coupons.columns')
            ->rejectUnlisted()
            ->values();

        if (Coupon::query()->count() === 0) {
            return view('cargo::cp.coupons.empty');
        }

        return view('cargo::cp.coupons.index', [
            'blueprint' => $blueprint,
            'columns' => $columns,
    ...
    public function create(Request $request)
    {
        ...
        $viewData = [
            ...
            'actions' => [
                'save' => cp_route('cargo.coupons.store'),
            ],
            'values' => $values->all(),
            'meta' => $fields->meta(),
            'blueprint' => $blueprint->toPublishArray(),
            'breadcrumbs' => new Breadcrumbs([
                ['text' => __('Coupons'), 'url' => cp_route('cargo.coupons.index')],
            ]),
        ];
        ...
        return view('cargo::cp.coupons.create', $viewData);
    }
    ...
    public function edit(Request $request, $coupon)
    {
        ...
        return view('cargo::cp.coupons.edit', array_merge($viewData, [
            'coupon' => $coupon,
        ]));
    }
